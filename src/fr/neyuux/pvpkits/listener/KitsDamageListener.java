	...
	private void checkWin() {
		if (Teams.getAliveTeams().size() == 1 || Teams.getAliveTeams().size() == 0) {
			...
			if (Teams.getAliveTeams().size() == 1) {
				...
				main.sendTitleForAllPlayers("§c§l§n§kaa §e§l§nVictoire de la team " + team.getColor() + team.getAdjectiveName() + " §c§l§n§kaa§r", "§6§l§nNombre de Survivants §7§l: §f" + main.players.size(), 20, 180, 20);
				...
				main.sendTitleForAllPlayers("§5§kaa §c§l§nÉgalité §5§kaa", "§cAucun survivant.", 20, 120, 20);
			}
		}
	}
